...
export async function postCreateAssignment(req: Request, res: Response): Promise<void> {
  ...
  const assignment = await createAssignment(parsed, user.id, schoolId);
...
export async function getListAssignments(req: Request, res: Response): Promise<void> {
  ...
  // Teachers see their own assignments by default; admins see all.
  if (user.role === 'teacher') query.teacherId = user.id;

  const result = await listAssignments(schoolId, query);
...
export async function getAssignment(req: Request, res: Response): Promise<void> {
  const schoolId = requireSchoolId(req, res);
  if (!schoolId) return;
  const assignment = await getAssignmentById(req.params.id as string, schoolId);
...
export async function putUpdateAssignment(req: Request, res: Response): Promise<void> {
  ...
  const assignment = await updateAssignment(req.params.id as string, schoolId, parsed);
...
export async function deleteAssignmentHandler(req: Request, res: Response): Promise<void> {
  const schoolId = requireSchoolId(req, res);
  if (!schoolId) return;
  await deleteAssignment(req.params.id as string, schoolId);
  res.status(204).end();
}
...
export async function getClassAssignments(req: Request, res: Response): Promise<void> {
  const schoolId = requireSchoolId(req, res);
  if (!schoolId) return;
  const list = await listClassAssignments(req.params.id as string, schoolId);
...
export async function getSubmissions(req: Request, res: Response): Promise<void> {
  const schoolId = requireSchoolId(req, res);
  if (!schoolId) return;
  const list = await listSubmissionsForAssignment(req.params.id as string, schoolId);
  res.json(apiResponse(true, list));
}
...
export async function getOneSubmission(req: Request, res: Response): Promise<void> {
  const schoolId = requireSchoolId(req, res);
  if (!schoolId) return;
  const sub = await getSubmissionById(req.params.submissionId as string, schoolId);
